  ...
  <div>
    ...
    <el-card>
      <el-table :data="noticeList" stripe style="width: 100%">
        ...
        <el-table-column prop="noticeTitle" label="通知标题" width="740">
          <template slot-scope="scope">
            <a style="color: blue; text-decoration: underline;" @click="viewContent(scope.row.noticeId, scope.row.noticeDesc)">{{
              scope.row.noticeTitle }}</a>.
          </template>
        </el-table-column>
        <el-table-column prop="schoolDesc" label="通知所属" width="120" />
        <el-table-column prop="noticeTime" label="通知时间" width="150" />
        <el-table-column label="操作" width="150" align="center">
          <template slot-scope="scope">
            <el-button type="primary" icon="el-icon-edit" circle
              @click="openEditUI(scope.row.noticeId, scope.row.noticeDesc)" />
...
export default {
  ...
  created() { // 构造函数
    this.searchModel.schoolId = 1 // 校级通知
    this.getNoticeList()
    editor = ''
    descEditor = ''
  },
