  ...
  <div>
    ...
    <el-card>
      <el-table :data="resultsList" stripe style="width: 100%">
        ...
        <el-table-column prop="resultsName" label="成果名" width="370" align="center" />
        <el-table-column prop="resultsChargeName" label="负责人" width="100" />
        <el-table-column prop="schoolDesc" label="成果所属" width="120" />
        <el-table-column prop="resultsSubject" label="成果学科" width="120" />
        <el-table-column prop="typeDesc" label="成果类别" width="100" />
        <el-table-column prop="statusDesc" label="成果状态" width="120">
          <template slot-scope="scope">
        ...
        <el-table-column label="操作" width="110">
          <template slot-scope="scope">
            <el-button type="primary" icon="el-icon-edit" circle size="mini"
              :style="{ display: scope.row.resultsStatusId == 10 ? '' : 'none' }"
              @click="openEditUI(scope.row.resultsId)" />
            <el-button type="danger" icon="el-icon-delete" circle size="mini"
              :style="{ display: scope.row.resultsStatusId == 10 ? '' : 'none' }" @click="deleteResults(scope.row)" />
            <el-button type="primary" icon="el-icon-search" circle size="mini"
              :style="{ display: scope.row.resultsStatusId == 10 ? 'none' : '' }"
              @click="openEditUI(scope.row.resultsId, scope.row.resultsStatusId)" />
            <el-button type="danger" size="mini" circle
              :style="{ display: scope.row.resultsStatusId == 11 ? '' : 'none' }"
              @click="withdraw(scope.row.resultsId)">撤回</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
    ...
    <el-dialog v-drag :title="title" :visible.sync="dialogFormVisible" :close-on-click-modal="false" @close="clearForm">
      <el-form ref="resultsFormRef" :model="resultsForm" :rules="rules">
        ...
        <el-row>
          ...
          <el-col :span="10">
            <el-form-item
              :style="{ display: resultsForm.resultsStatusId == null || resultsForm.resultsStatusId == 10 ? 'none' : '' }"
              label="成果状态" :label-width="formLabelWidth">
...
export default {
  ...
  methods: {
    ...
    openEditUI(id, resultsStatusId) {
      if (id == null) {
        this.title = '新增成果'
      } else if (resultsStatusId === 10) {
        this.title = '修改成果'
        // 根据ID查询成果
    ...
    withdraw(id) {
      ...
      this.resultsForm.resultsStatusId = 10
