  ...
  <div>
    ...
    <el-card id="search">
      <el-row>
        ...
        <el-col :span="4" align="right">
          <el-button type="primary" icon="el-icon-plus" circle @click="openEditUI(null, null)" />
        </el-col>
      </el-row>
    </el-card>
    <!--结果列表-->
    <el-card>
      <el-table :data="proList" stripe style="width: 100%">
        ...
        <el-table-column prop="proName" label="项目名" width="150" />
        <el-table-column prop="proChargeName" label="负责人" width="70" />
        <el-table-column prop="schoolDesc" label="项目所属" width="120" />
        <el-table-column prop="proSubject" label="项目学科" width="120" />
        <el-table-column prop="resultsTypeDesc" label="成果类别" width="90" />
        <el-table-column prop="statusDesc" label="项目状态" width="130">
          <template slot-scope="scope">
            <el-tag>{{ scope.row.statusDesc }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="members" label="项目成员" width="120" />
        <el-table-column prop="budget" label="项目预算" width="90" />
        <el-table-column label="相关文件" width="90">
          <template slot-scope="scope">
            <el-button icon="el-icon-search" circle @click="openFileUI(scope.row.proId)" />
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="110">
          <template slot-scope="scope">
            <el-button type="primary" icon="el-icon-edit" circle size="mini"
              :style="{ display: scope.row.proStatusId == 1 ? '' : 'none' }"
              @click="openEditUI(scope.row.proId, scope.row.proStatusId)" />
            <el-button type="danger" icon="el-icon-delete" circle size="mini"
              :style="{ display: scope.row.proStatusId == 1 ? '' : 'none' }" @click="deletePro(scope.row)" />
            <el-button type="primary" icon="el-icon-search" circle size="mini"
              :style="{ display: scope.row.proStatusId == 1 ? 'none' : '' }"
              @click="openEditUI(scope.row.proId, scope.row.proStatusId)" />
            <el-button type="danger" size="mini" circle
              :style="{ display: scope.row.proStatusId == 2 || scope.row.proStatusId == 5 || scope.row.proStatusId == 7 ? '' : 'none' }"
              @click="withdraw(scope.row.proId, scope.row.proStatusId)">撤回</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
    ...
    <el-dialog :title="title" :visible.sync="dialogFormVisible" :close-on-click-modal="false" @close="clearForm">
      <el-form ref="proFormRef" :model="proForm" :rules="rules">
        ...
        <el-row>
          ...
          <el-col :span="10">
            <el-form-item :style="{ display: proForm.proStatusId == null || proForm.proStatusId == 1 ? 'none' : '' }"
              label="项目状态" :label-width="formLabelWidth">
    ...
    <el-dialog v-drag :title="fileTitle" :visible.sync="dialogFileFormVisible" :close-on-click-modal="false"
      ...
      <el-form ref="proFileFormRef" :model="proForm">
        ...
        <el-form-item label="项目申报书" :label-width="formLabelWidth" prop="proProposalUrl">
          ...
          <el-upload :action="'http://' + serverIp + ':9999/files/upload'" :show-file-list="false"
            :on-success="handleFileUploadProProposalSuccess" style="display:inline">
            <el-button :style="{ display: proForm.proStatusId > 1 ? 'none' : '' }" type="primary"
              style="margin-left: 10px;">上传<i class="el-icon-top" /></el-button>
          </el-upload>
          <el-button :style="{ display: proForm.proProposalUrl == null ? 'none' : '' }" type="primary"
        ...
        <el-form-item label="中期检查报告" :label-width="formLabelWidth" prop="interInspectUrl">
          ...
          <el-upload :action="'http://' + serverIp + ':9999/files/upload'" :show-file-list="false"
            :on-success="handleFileUploadInterInspectSuccess" style="display:inline">
            <el-button :style="{ display: proForm.proStatusId > 5 ? 'none' : '' }" type="primary"
              style="margin-left: 10px;">上传<i class="el-icon-top" /></el-button>
          </el-upload>
        ...
        <el-form-item label="项目结项书" :label-width="formLabelWidth" prop="proEndfileUrl">
          ...
          <el-upload :action="'http://' + serverIp + ':9999/files/upload'" :show-file-list="false"
            ...
            <el-button :style="{ display: proForm.proStatusId >= 7 ? 'none' : '' }" type="primary"
              style="margin-left: 10px;">上传<i class="el-icon-top" /></el-button>
          </el-upload>
...
export default {
  ...
  methods: {
    ...
    openEditUI(id, proStatusId) {
      if (id == null) {
        this.title = '新增项目'
      } else if (proStatusId === 1) {
        this.title = '修改项目'
        // 根据ID查询项目
    ...
    withdraw(id, proStatusId) {
      this.proForm.proId = id
      if (proStatusId === 2) {
        this.proForm.proStatusId = 1
      } else if (proStatusId === 5) {
        this.proForm.proStatusId = 4
      } else if (proStatusId === 7) {
        this.proForm.proStatusId = 6
      }
    ...
    saveProFile() {
      // 提交给后台
      if (this.proForm.proStatusId === 1) { // 是草稿才可修改
        this.proForm.updateTimeString = this.getCurrentDateTime() // 修改时间
      }
      proApi.savePro(this.proForm).then(response => {
        // 成功提示
    ...
    handleFileUploadProProposalSuccess(res) {
      this.proForm.proProposalUrl = res
      this.$message({
        type: 'success',
        message: '上传成功'
    ...
    handleFileUploadInterInspectSuccess(res) {
      this.proForm.interInspectUrl = res
      this.$message({
        type: 'success',
        message: '上传成功'
      })
