      ...
      <el-form ref="resultsFormRef" :model="resultsForm">
        ...
        <el-form-item label="成果证明文件" :label-width="formLabelWidth" prop="resultsUrl">
          <el-input v-model="resultsForm.resultsUrl" style="width:440px" placeholder="如有多个文件，请以压缩包的格式上传"
            autocomplete="off" />
          <el-button :style="{ display: resultsForm.resultsUrl == null ? 'none' : '' }" type="primary" style="margin-left: 10px;" @click="download(resultsForm.resultsUrl)">下载<i
  ...
  methods: {
      ...
      getPersonInfo(token).then(response => {
        ...
        userApi.getAllSciUserList().then(userRes => {
          ...
          for (let i = 0; i < this.userList.length; i++) {
            if (this.userList[i].userSchoolId !== this.searchModel.resultsSchoolId) { // 删除所有不是本学院的负责人
    ...
    async getExcel() {
      try {
        ...
        link.href = url
        link.setAttribute('download', this.schoolDesc + '成果表.xlsx')
