  ...
  <div>
    ...
    <el-card id="search">
      <el-row>
        <el-col :span="22">
          ...
          <el-select v-model="searchModel.proStatusId" placeholder="请选择项目状态" filterable clearable>
            <el-option v-for="item in statusList" :key="item.statusId" :label="item.statusDesc" :value="item.statusId" />
          </el-select>
          <el-button type="primary" icon="el-icon-search" round @click="getProList">查询</el-button>
        </el-col>
    ...
    <el-card>
      <el-table :data="proList" stripe style="width: 100%">
        ...
        <el-table-column prop="proName" label="项目名" width="140" />
        <el-table-column prop="proChargeName" label="负责人" width="110" />
        <el-table-column prop="schoolDesc" label="项目所属" width="140" />
        <el-table-column prop="proSubject" label="项目学科" width="140" />
        <el-table-column prop="resultsTypeDesc" label="成果类别" width="120" />
        <el-table-column prop="statusDesc" label="项目状态" width="160">
            <template slot-scope="scope">
    ...
    <el-dialog :title="title" :visible.sync="dialogFormVisible" :close-on-click-modal="false" @close="clearForm">
      <el-form ref="proFormRef" :model="proForm">
        ...
        <el-form-item label="项目申报书" :label-width="formLabelWidth" prop="proProposalUrl">
          ...
          <el-button :style="{ display: proForm.proProposalUrl == null ? 'none' : '' }" type="primary" style="margin-left: 10px;" @click="download(proForm.proProposalUrl)">下载<i
              class="el-icon-download" /></el-button>
        </el-form-item>
        <el-form-item :style="{ display: proForm.proStatusId < 5 ? 'none' : '' }" label="中期检查报告"
          :label-width="formLabelWidth" prop="interInspectUrl">
          <el-input v-model="proForm.interInspectUrl" style="width:440px" autocomplete="off" />
          <el-button :style="{ display: proForm.interInspectUrl == null ? 'none' : '' }" type="primary" style="margin-left: 10px;" @click="download(proForm.interInspectUrl)">下载<i
              class="el-icon-download" /></el-button>
        </el-form-item>
        <el-form-item :style="{ display: proForm.proStatusId < 7 ? 'none' : '' }" label="项目结项书"
          :label-width="formLabelWidth" prop="proEndfileUrl">
          <el-input v-model="proForm.proEndfileUrl" style="width:440px" autocomplete="off" />
          <el-button :style="{ display: proForm.proEndfileUrl == null ? 'none' : '' }" type="primary" style="margin-left: 10px;" @click="download(proForm.proEndfileUrl)">下载<i
              class="el-icon-download" /></el-button>
        </el-form-item>
      </el-form>
  ...
  methods: {
    getUserInfo() {
      ...
      getPersonInfo(token).then(response => {
        ...
        schoolApi.getSchoolById(this.schoolId).then(schoolRes => {
          this.schoolDesc = schoolRes.data.schoolDesc
        })
        this.getProList()
        // 获取本学院科研人员
        userApi.getAllSciUserList().then(userRes => {
          this.userList = userRes.data
          for (let i = 0; i < this.userList.length; i++) {
            if (this.userList[i].userSchoolId !== this.searchModel.proSchoolId) { // 删除所有不是本学院的负责人
    ...
    getProList() {
      proApi.getProList(this.searchModel).then(response => { // then是回调函数
    ...
    getProStatusExceptDraft() {
      statusApi.getProStatusExceptDraft().then(response => {
    ...
    async getExcel() {
      try {
        const excelModel = {
          proChargeId: '',
          proSchoolId: this.schoolId,
          proStatusId: '1'
        }
        const response = await proApi.getExcelFile(excelModel)
        const url = window.URL.createObjectURL(new Blob([response.data]))
        const link = document.createElement('a')
        link.href = url
        link.setAttribute('download', this.schoolDesc + '项目表.xlsx')
